  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Crear lista de participantes con icono de eliminar y sin bullets
                ...
                ${details.participants
                  .map(
                    ...
                    <li style="display: flex; align-items: center; gap: 6px;">
                      <span>${email}</span>
                      <span class="delete-participant" title="Unregister" data-activity="${encodeURIComponent(
                        name
                      )}" data-email="${encodeURIComponent(
                      email
                    )}" style="cursor:pointer;color:#c00;font-weight:bold;">&#10006;</span>
                    </li>
                  `
                  )
                  .join("")}
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        signupForm.reset();
        fetchActivities(); // Actualizar la lista de actividades sin refrescar la página
      } else {
        ...
        try {
          ...
          if (response.ok) {
            messageDiv.textContent =
              result.message || "Participant unregistered.";
            messageDiv.className = "success";
            fetchActivities();
          } else {
            messageDiv.textContent = result.detail || "An error occurred";
